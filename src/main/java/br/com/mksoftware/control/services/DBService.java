...
public class DBService {
	...
	public void dbInit() {
		...
		Set<TagPermission> taglist = new HashSet<TagPermission>();
		Function pAdmin = new Function(null, "Admin", "usuario administrador do sistema", true, taglist);
		Function pBasic = new Function(null, "Basic", "usuario basico do sistema", true, taglist);
		Function pInsert = new Function(null, "Insert", "usuario para operações de insert", true, taglist);
		Function pUpdate = new Function(null, "Update", "usuario para operações de update", true, taglist);
		Function pDelete = new Function(null, "Delete", "usuario para operações de delete", true, taglist);
		...
		userBasic.setFirstName("userBasic");
		userBasic.setLastName("userBasic");
		userBasic.setIdentityDocument("0976905973");
		userBasic.setIsActive(true);
		userBasic.setMail("userbasic@gmail.com");
		userBasic.setNickname("userBasic");
		userBasic.setSocialNumber("01825809543");
		userBasic.setBirthDate(OffsetDateTime.now());
		userBasic.setUsername("admin");
		userBasic.setPassword(encode.encode("123456"));
		userBasic.setDepartment(dp2);
		userBasic.setOfficer(of2);
		userBasic.getFunctionList().add(pBasic);
		userBasic.getContactList().add(new Contact(null, ContactTypeEnum.CEL, "71", "991637645"));
		userBasic.getSystemList().addAll(Arrays.asList(sys1));
		
		User userInsert = new User();
		userInsert.setFirstName("userInsert");
		userInsert.setLastName("userInsert");
		userInsert.setIdentityDocument("0976905974");
		userInsert.setIsActive(true);
		...
		userUpdate.setDepartment(dp1);
		userUpdate.setOfficer(of3);
		userUpdate.getFunctionList().add(pUpdate);
		userUpdate.getContactList().add(new Contact(null, ContactTypeEnum.CEL, "71", "991637645"));
		userUpdate.getSystemList().addAll(Arrays.asList(sys1,sys2,sys4));
		
		userAdminRepository.saveAll(Arrays.asList(userAdmin, userBasic, userInsert,userUpdate));
		
		

		
		//contactRepository.saveAll(Arrays.asList(cont1,cont2,cont3));
